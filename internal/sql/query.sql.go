...
func (q *Queries) CleanSpans(ctx context.Context, pruneBefore time.Time) (int64, error) {
	result, err := q.db.ExecContext(ctx, cleanSpans, pruneBefore)
	if err != nil {
		return 0, err
	}
...
type FindTraceIDsParams struct {
	...
	StartTimeMinimum             time.Time
	StartTimeMinimumEnableFilter bool
	StartTimeMaximum             time.Time
	StartTimeMaximumEnableFilter bool
	DurationMinimum              int64
...
func (q *Queries) FindTraceIDs(ctx context.Context, arg FindTraceIDsParams) ([][]byte, error) {
	rows, err := q.db.QueryContext(ctx, findTraceIDs,
		arg.ServiceName,
...
func (q *Queries) GetOperationID(ctx context.Context, arg GetOperationIDParams) (int64, error) {
	row := q.db.QueryRowContext(ctx, getOperationID, arg.Name, arg.ServiceID, arg.Kind)
	var id int64
	err := row.Scan(&id)
	return id, err
}
...
func (q *Queries) GetOperations(ctx context.Context, serviceName string) ([]GetOperationsRow, error) {
	rows, err := q.db.QueryContext(ctx, getOperations, serviceName)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) GetSpansCount(ctx context.Context) (int64, error) {
	row := q.db.QueryRowContext(ctx, getSpansCount)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
func (q *Queries) GetSpansDiskSize(ctx context.Context) (int64, error) {
	row := q.db.QueryRowContext(ctx, getSpansDiskSize)
...
func (q *Queries) UpsertOperation(ctx context.Context, arg UpsertOperationParams) error {
	_, err := q.db.ExecContext(ctx, upsertOperation, arg.Name, arg.ServiceID, arg.Kind)
	return err
}
...
WITH insert_if_not_exists AS (
    SELECT id FROM services WHERE name = $1::VARCHAR
)
