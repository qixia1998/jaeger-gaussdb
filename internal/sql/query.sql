...
WITH insert_if_not_exists AS (
    SELECT id FROM services WHERE name = sqlc.arg(name)::VARCHAR
)
INSERT INTO services (name)
SELECT sqlc.arg(name)::VARCHAR
WHERE NOT EXISTS (SELECT 1 FROM insert_if_not_exists)
RETURNING id;
...
WITH insert_if_not_exists AS (
    ...
    FROM operations
    WHERE name = sqlc.arg(name)::TEXT
    AND service_id = sqlc.arg(service_id)::BIGINT
    AND kind = sqlc.arg(kind)::SPANKIND
    )
...
SELECT
  ...
  CAST(EXTRACT(EPOCH FROM duration) * 1000 AS BIGINT) as duration,
  spans.tags as tags,
