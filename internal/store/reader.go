...
// Reader can query for and load traces from GaussDB
type Reader struct {
	logger *slog.Logger
	q      *sql.Queries
}

// NewReader returns a new SpanReader for GaussDB
...
func (r *Reader) GetTrace(ctx context.Context, traceID model.TraceID) (*model.Trace, error) {
	...
	for i, dbSpan := range dbSpans {
		tags, err := DecodeTags(dbSpan.Tags.RawMessage)
		if err != nil {
		...
		duration := time.Duration(dbSpan.Duration * 1000)

		logs, err := DecodeLogs(dbSpan.Logs.RawMessage)
		if err != nil {
...
func (r *Reader) FindTraces(ctx context.Context, query *spanstore.TraceQueryParameters) ([]*model.Trace, error) {
	...
	response, err := r.q.FindTraceIDs(ctx, sql.FindTraceIDsParams{
		...
		DurationMinimum:              int64(EncodeInterval(query.DurationMin)),
		DurationMinimumEnableFilter:  query.DurationMin != time.Duration(0),
		DurationMaximum:              int64(EncodeInterval(query.DurationMax)),
